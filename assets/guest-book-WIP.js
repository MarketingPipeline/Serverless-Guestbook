...
fetch(
  ...
  // Removing any inputs with illegal characters / words
 // That got through 
         // ie: Do not show any inputs that are still deleting 
    // from google spreadsheet
    if (validate_text(SantizeResponses, SantizeName) === true) {
  document.getElementById("json").innerHTML += `
    ...
    if (validate_text(SantizeResponses, SantizeName) === false) {
  // do nothing 
} 
	...
	/// Adding all entries to all entry section
        ...
        var validate_text = validate_text(SantizeResponses)  
...
Gform.addEventListener('submit', (e) => {
  validateRecaptcha();
	var GuestBook_Entry_Value = document.getElementById(GOOGLE_ENTRY_ID_Guestbook).value
...
function validateRecaptcha() {
    ...
    if (response.length === 0) {
        ...
        validate_text(GuestBook_Entry_Value);
        return true;
    }
}
